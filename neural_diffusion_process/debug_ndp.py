...
def debug_ndp_eval(cfg, dataloader, diffusion, ema_model, results_dir=0, epoch=0,
    ...
    for (x_grid, y_val, _) in dataloader:
        ...
        with torch.no_grad():
            ...
            context_loss = loss * context_only.to(device)
            context_loss = context_loss.sum() / context_only.to(device).sum()
            context_loss_array.append(context_loss.item())
            loss = loss * (1 - context_only.to(device))
            loss = loss.sum() / (1 - context_only.to(device)).sum()
            loss_array.append(loss.item())
        
    loss = torch.tensor(loss_array).mean()
    context_loss = torch.tensor(context_loss_array).mean()
    print(f"Loss: {loss}, Context Loss: {context_loss}")
...
def main(cfg):
    ...
    model.load_state_dict(torch.load("/home/users/asubedi/neural_processes/neural_processes/neural_diffusion_process/working_dir/models/third_run/ckpt_119.pt"))
    debug_ndp_eval(cfg, val_dataloader, diffusion, model)
